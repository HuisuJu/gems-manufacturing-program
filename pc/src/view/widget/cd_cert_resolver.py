...
class CDCertResolverFrame(ctk.CTkFrame):
    """
    GUI frame for selecting and validating a Certification Declaration file.

    This widget does not use global settings.
    It owns the currently loaded file path internally.

    Features:
        - Select CD file from file dialog
        - Load CD file
    ...
    def __init__(self, master, **kwargs) -> None:
        ...
        self._title_label = ctk.CTkLabel(
            self,
            text="Certification Declaration File",
            anchor="w",
        ...
        self._status_label = ctk.CTkLabel(
            self,
            text="No Certification Declaration file loaded.",
            anchor="w",
            justify="left",
            wraplength=700,
        )
    ...
    def path(self) -> Path | None:
        """
        Return the currently loaded CD file path.
        """
        return self._path

    def get_path(self) -> Path:
        """
        Return the configured CD file path.

        Raises:
            AttestationPathResolverConfigurationError:
                If the CD file has not been loaded.
        """
    ...
    def load_path(self, path: Path | None) -> None:
        """
        Load a Certification Declaration file path into the widget.

        Args:
            path: CD file path, or None to clear current selection.
        """
        if path is None:
            self._path = None
            self._path_entry.delete(0, "end")
            self._set_status("No Certification Declaration file loaded.")
            return
        ...
        if not resolved.exists():
            raise AttestationPathResolverConfigurationError(
                f"The Certification Declaration file does not exist: '{resolved}'."
            )

        if not resolved.is_file():
            raise AttestationPathResolverConfigurationError(
                f"The Certification Declaration path is not a file: '{resolved}'."
            )
        ...
        self._set_status(
            f"Certification Declaration file loaded successfully: {resolved}"
        )
    ...
    def _on_load(self) -> None:
        ...
        if not raw_path:
            self._show_error("Please select a Certification Declaration file first.")
            return
    ...
    def _on_refresh(self) -> None:
        """
        Reload the currently loaded CD file.
        """
        if self._path is None:
            self._show_error("There is no Certification Declaration file to refresh.")
            return
    ...
    def _on_clear(self) -> None:
        """
        Clear current CD file selection.
        """
        self.load_path(None)
