...
manualcode_format = BitStruct(
    ...
    "pid" / BitsInteger(16),
    "padding"
    / BitsInteger(7),  # this is intentional as BitStruct only takes 8-bit aligned data
)
...
class SetupPayload:
    ...
    def p_print(self):
        ...
        if self.long_discriminator:
            print(
                "{:<{}} :{}".format("Long Discriminator", 24, self.long_discriminator)
            )
        if self.discovery:
        ...
        if self.vid is not None and self.pid is not None:
            print(
                "{:<{}} :{:<{}} (0x{:04x})".format(
                    "Vendor Id", 24, self.vid, 6, self.vid
                )
            )
            print(
                "{:<{}} :{:<{}} (0x{:04x})".format(
                    "Product Id", 24, self.pid, 6, self.pid
                )
            )
    ...
    def generate_manualcode(self):
        ...
        chunk1 = str(
            int(bits[CHUNK1_START : CHUNK1_START + CHUNK1_LEN].to01(), 2)
        ).zfill(1)
        chunk2 = str(
            int(bits[CHUNK2_START : CHUNK2_START + CHUNK2_LEN].to01(), 2)
        ).zfill(5)
        chunk3 = str(
            int(bits[CHUNK3_START : CHUNK3_START + CHUNK3_LEN].to01(), 2)
        ).zfill(4)
        chunk4 = (
            str(self.vid).zfill(5) if self.flow != CommissioningFlow.Standard else ""
        )
        chunk5 = (
            str(self.pid).zfill(5) if self.flow != CommissioningFlow.Standard else ""
        )
...
@click.option(
    ...
    "-p",
    required=True,
    type=click.IntRange(1, 0x5F5E0FE),
    help="setup pincode",
)
@click.option(
    "--vendor-id", "-vid", type=click.IntRange(0, 0xFFFF), default=0, help="Vendor ID"
)
@click.option(
    "--product-id", "-pid", type=click.IntRange(0, 0xFFFF), default=0, help="Product ID"
)
@click.option(
