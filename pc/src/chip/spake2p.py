...
def generate_verifier(passcode: int, salt: bytes, iterations: int) -> bytes:
    ws = hashlib.pbkdf2_hmac(
        "sha256", struct.pack("<I", passcode), salt, iterations, WS_LENGTH * 2
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SPAKE2+ Python Tool", fromfile_prefix_chars="@"
    )
    commands = parser.add_subparsers(
        dest="command", metavar="subcommand".ljust(16), required=True
    )

    gen_verifier = commands.add_parser("gen-verifier", help="Generate SPAKE2+ Verifier")
    gen_verifier.add_argument(
        "-p", "--passcode", type=passcode_arg, required=True, help="8-digit passcode"
    )
    gen_verifier.add_argument(
        "-s",
        "--salt",
        type=salt_arg,
        required=True,
        help="Salt of length 16 to 32 octets encoded in Base64",
    )
    gen_verifier.add_argument(
        "-i",
        "--iteration-count",
