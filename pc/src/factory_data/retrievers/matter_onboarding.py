...
SPAKE2P_VERIFIER_TOTAL_SIZE = (
    SPAKE2P_VERIFIER_W0_SIZE + SPAKE2P_VERIFIER_L_SIZE
)


class MatterOnboardingDataRetriever(Retriever):
    """Generate Matter onboarding data."""
    ...
    def _generate_passcode(self) -> int:
        ...
        while True:
            passcode = PASSCODE_MIN + secrets.randbelow(
                PASSCODE_MAX - PASSCODE_MIN + 1
            )
    ...
    def _split_verifier(self, verifier: bytes) -> tuple[str, str]:
        ...
        if len(verifier) != SPAKE2P_VERIFIER_TOTAL_SIZE:
            raise RetrieverError(
                "SPAKE2+ verifier has an unexpected length."
            )
